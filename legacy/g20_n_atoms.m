...
% U(1,1)=1;
% for i=1:N
%  U(i+1,2:end)=flip(rootsOfUnity(N))'.^(i-1)/sqrt(N);
% end
...
k=1;   %  4.3 - Q~100,000 , 43 - Q~10,000  %cavity linewidth
k1=k/2;
k2=k/2;
g=.01;   % emitter linewidth
go=1;
...
%         gw2(i)=gnd'*a1*phi1*D1*phi1v*a1'*a1*phi1*D1*phi1v*a1'*gnd;
        ...
        g2w(i)=gnd'*a1*phi1*phi1v*a2*phi2*D2*phi2v*a2'*phi1*D1*phi1v*a1'*gnd;
        %third order
        if g3fig==1
...
if showfig1==1
    ...
    ylabel('Transmission Amplitude','FontSize',24)
...
% figure(10)
% plotNum=length(n);
% subplot(plotNum,1,n_index)
% hold on
% plot(W-wc,T2.*conj(T2))
% plot(W-wc,g2_w)
